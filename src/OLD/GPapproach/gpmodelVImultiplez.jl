####################################################################
function gpmodelVImultiplez(x, flux, σ2, idx; maxiterml = 10, maxitervi = 10, maxrandom=10, seed = 1, show_trace = true, S=S,Stest=0)
####################################################################
    ...
    #--------------------------------------------------------
    function logl0(param)
    #--------------------------------------------------------

        local S = length(param)

        local ℓ = 0.0

        for s in 1:S

            @inbounds local a, b = param[s][1:3], param[s][4:6]
            ...
            ℓ += - 0.5*logdetC - 0.5*yTCinvy - 0.5*N*log(2.0π)
    ...
    # initsolutions       = [randn(rg, 7) for i=1:1000]
    ...
    posterior, logevidence = VI(logl0, [a;b], S=S, Stest=Stest, show_every=10, iterations=maxitervi, optimiser = NelderMead())
