####################################################################
function gpmodelVI(x, flux, idx; maxiterml = 10, maxitervi = 10, maxrandom=10, seed = 1, show_trace = true, S=S)
####################################################################
    ...
    #--------------------------------------------------------
        ...
        local a     = param[1+0*numF:1*numF]
        local b     = param[1+1*numF:2*numF]
        local logσ2 = param[1+2*numF:3*numF]
        local logl  = param[1+3*numF]
    ...
    #--------------------------------------------------------
        ...
        local a, b, logσ2, logl = unpack(param)

        -1.0 * logp(a = a, b = b, logσ2 = logσ2, logl = logl)
    ...
    #--------------------------------------------------------
    function logp(; a = a, b = b, logσ2 = logσ2, logl = logl)
    #--------------------------------------------------------
    ...
    a, b, logσ2, logl = unpack(result.minimizer)
    ...
    newobjective(x) = logp0(a = x[1:numF], b = x[numF+1:end])


    opt = Optim.Options(show_trace = true, show_every = 10, iterations = maxitervi, g_tol=1e-6)

    mu, Sigma, nev = VI(newobjective, [a;b], S=S, maxiter=maxitervi, optimiser = NelderMead(), optimoptions = opt)
    ...
    return MvNormal(mu, 0.5*(Sigma + Sigma')), exp.(logσ2)
